...
export async function GET(req: NextRequest) {
  try {
    ...
    const [
      ...
      supabase.from('expenses').select('value').ilike('created_at', `${filterPrefix}%`),
      supabase.from('expenses').select('value').ilike('created_at', `${prevFilterPrefix}%`),
      supabase.from('shopping_sessions').select('total_cost').ilike('date', `${filterPrefix}%`),
      supabase.from('expenses').select('value').eq('paid', false),
      supabase.from('expenses').select('id, name, value, due_date, supplier_id').eq('paid', false).not('due_date', 'is', null).lt('due_date', today).order('due_date').limit(10),
      ...
      supabase.from('expenses').select('created_at, value').gte('created_at', new Date(Date.now() - 7 * 30 * 24 * 3600 * 1000).toISOString()),
      ...
      supabase.from('expenses').select('category, value, paid').ilike('created_at', `${filterPrefix}%`),
      supabase.from('expenses').select('category, value, paid'),
